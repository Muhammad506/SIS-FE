...
const generateBatteryHistory = () => {
  return Array.from({ length: 24 }, (_, i) => ({
    time: `${i.toString().padStart(2, "0")}:00`,
    batteryLevel: Math.min(100, Math.max(0, 50 + (Math.random() * 50 - 25))),
  }));
};
...
const ActivePlates = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setPlates((prev) => {
        const newPlates = prev.map((plate) => ({
          ...plate,
          active: Math.random() > 0.9 ? !plate.active : plate.active,
          powerOutput:
            Math.random() > 0.9 && !plate.active
      ...
      setBatteryLevel((prev) => {
        const activePlates = plates.filter((plate) => plate.active).length;
        const chargeRate =
          activePlates > 0 ? Math.random() * 5 : -Math.random() * 2;
        return Math.min(100, Math.max(0, prev + chargeRate));
      });
